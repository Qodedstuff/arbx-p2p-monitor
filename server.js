...
async function fetchBybit(side, currency = 'USDT') {
  ...
  try {
    ...
    const filtered = items.filter(item => {
      ...
      // Check ad can handle between $100 and $1000 worth of NGN
      const dollarMin = price * 100;   // ~$100 in NGN
      const dollarMax = price * 1000;  // ~$1000 in NGN
      const adMin = parseFloat(item.minAmount || 0);
      const adMax = parseFloat(item.maxAmount || 999999999);
      // Ad must accept at least some amount within our $100-$1000 range
      const withinRange = adMax >= dollarMin && adMin <= dollarMax;
...
app.get('/api/analytics/:currency?', (req, res) => {
  ...
  // Find best hours - need at least 2 data points in different hours to be meaningful
  const withSellData = hourlyStats.filter(h => h.bybit.avgSell !== null || h.gate.avgSell !== null);
  const withBuyData = hourlyStats.filter(h => h.bybit.avgBuy !== null || h.gate.avgBuy !== null);

  // Best hour to BUY = hour with lowest average SELL price
  const bestBuyHour = withSellData.length >= 2
    ? withSellData.reduce((best, h) => {
        const sell = h.bybit.avgSell ?? h.gate.avgSell;
        const bestSell = best ? (best.bybit.avgSell ?? best.gate.avgSell) : null;
        if (sell === null) return best;
        if (bestSell === null || sell < bestSell) return h;
        return best;
      }, null)
    : null;

  // Best hour to SELL = hour with highest average BUY price
  const bestSellHour = withBuyData.length >= 2
    ? withBuyData.reduce((best, h) => {
        const buy = h.bybit.avgBuy ?? h.gate.avgBuy;
        const bestBuy = best ? (best.bybit.avgBuy ?? best.gate.avgBuy) : null;
        if (buy === null) return best;
        if (bestBuy === null || buy > bestBuy) return h;
        return best;
      }, null)
    : null;
